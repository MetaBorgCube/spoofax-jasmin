...
public class ClassGenerator {
    ...
    public byte[] generateClass(Context context, IStrategoTerm versionTerm, IStrategoTerm flagsTerm,
        IStrategoTerm nameTerm, IStrategoTerm signatureTerm, IStrategoTerm superNameTerm,
        IStrategoTerm interfacesTerm, IStrategoTerm fieldsTerm, IStrategoTerm methodsTerm) {
    ...
    private void generateField(ClassWriter cw, Context context, IStrategoTerm flagsTerm,
        IStrategoTerm nameTerm, IStrategoTerm descTerm, IStrategoTerm signatureTerm,
    ...
    private void generateMethod(ClassWriter cw, Context context, IStrategoTerm flagsTerm,
        IStrategoTerm nameTerm, IStrategoTerm descTerm, IStrategoTerm signatureTerm,
    ...
    private void generateInstruction(MethodVisitor mv, Context context, Map<String, Label> labels,
        IStrategoTerm instrTerm) {
        if(isTermAppl(instrTerm)) {
            ...
            if(hasConstructor(instrAppl, "Maxs", 2)) {
                ...
                mv.visitFieldInsn(javaIntAt(instrAppl, 0), javaStringAt(instrAppl, 1),
                    javaStringAt(instrAppl, 2), javaStringAt(instrAppl, 3));
            } else if(hasConstructor(instrAppl, "MethodInsn", 4)) {
                mv.visitMethodInsn(javaIntAt(instrAppl, 0), javaStringAt(instrAppl, 1),
                    javaStringAt(instrAppl, 2), javaStringAt(instrAppl, 3));
                ...
                throw new IllegalArgumentException("Invalid instruction term ("
                    + instrAppl.getConstructor().toString() + ").");
            }
        } else {
            throw new IllegalArgumentException("Invalid instruction term.");
        }
    ...
    private Object termToConstant(IStrategoTerm term) {
        if(isTermAppl(term)) {
            ...
            if(hasConstructor(appl, "String", 1)) {
                ...
                throw new IllegalArgumentException("Unknown constant term constructor "
                    + appl.getConstructor() + ".");
            }
        ...
        /*
         * if (cst instanceof Integer) { // ... } else if (cst instanceof Float) { // ... } else if (cst
         * instanceof Long) { // ... } else if (cst instanceof Double) { // ... } else if (cst instanceof
         * String) { // ... } else if (cst instanceof Type) { int sort = ((Type) cst).getSort(); if (sort ==
         * Type.OBJECT) { // ... } else if (sort == Type.ARRAY) { // ... } else if (sort == Type.METHOD) { //
         * ... } else { // throw an exception } } else if (cst instanceof Handle) { // ... } else { // throw
         * an exception }
         */
    }
