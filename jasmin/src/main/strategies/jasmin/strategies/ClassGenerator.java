...
public class ClassGenerator {
    public byte[] generateClass(Context context, IStrategoTerm term) {
        if(isTermAppl(term) && hasConstructor((IStrategoAppl) term, "ASMClass", 8)) {
            ...
            return generateClass(context, versionTerm, flagsTerm, nameTerm, signatureTerm, superNameTerm,
                interfacesTerm, fieldsTerm, methodsTerm);
        } else {
            throw new IllegalArgumentException("Invalid class term.");
        }
    }

    public byte[] generateClass(Context context, IStrategoTerm versionTerm, IStrategoTerm flagsTerm,
        IStrategoTerm nameTerm, IStrategoTerm signatureTerm, IStrategoTerm superNameTerm, IStrategoTerm interfacesTerm,
        IStrategoTerm fieldsTerm, IStrategoTerm methodsTerm) {
        ...
        String superName = isNone(superNameTerm) ? null : javaString(superNameTerm);
    ...
    private void generateField(ClassWriter cw, Context context, IStrategoTerm field) {
    ...
    private void generateField(ClassWriter cw, Context context, IStrategoTerm flagsTerm, IStrategoTerm nameTerm,
        IStrategoTerm descTerm, IStrategoTerm signatureTerm, IStrategoTerm initValueTerm) {
        int flags = javaInt(flagsTerm);
        String name = javaString(nameTerm);
    ...
    private void generateMethod(ClassWriter cw, Context context, IStrategoTerm method) {
    ...
    private void generateMethod(ClassWriter cw, Context context, IStrategoTerm flagsTerm, IStrategoTerm nameTerm,
        IStrategoTerm descTerm, IStrategoTerm signatureTerm, IStrategoTerm exceptionsTerm, IStrategoTerm codeTerm) {
        int flags = javaInt(flagsTerm);
        String name = javaString(nameTerm);
        String desc = javaString(descTerm);
        String signature = isNone(signatureTerm) ? null : javaString(signatureTerm);
    ...
    @SuppressWarnings("deprecation") private void generateInstruction(MethodVisitor mv, Context context,
        ...
        if(isTermAppl(instrTerm)) {
            ...
            if(hasConstructor(instrAppl, "Maxs", 2)) {
                ...
                mv.visitFieldInsn(javaIntAt(instrAppl, 0), javaStringAt(instrAppl, 1), javaStringAt(instrAppl, 2),
                    javaStringAt(instrAppl, 3));
            } else if(hasConstructor(instrAppl, "MethodInsn", 4)) {
                mv.visitMethodInsn(javaIntAt(instrAppl, 0), javaStringAt(instrAppl, 1), javaStringAt(instrAppl, 2),
                    javaStringAt(instrAppl, 3));
                ...
                throw new IllegalArgumentException(
                    "Invalid instruction term (" + instrAppl.getConstructor().toString() + ").");
            }
        } else {
    ...
    private Object termToConstant(IStrategoTerm term) {
        ...
        /*
         * if (cst instanceof Integer) { // ... } else if (cst instanceof Float) { // ... } else if (cst instanceof
         * Long) { // ... } else if (cst instanceof Double) { // ... } else if (cst instanceof String) { // ... } else
         * if (cst instanceof Type) { int sort = ((Type) cst).getSort(); if (sort == Type.OBJECT) { // ... } else if
         * (sort == Type.ARRAY) { // ... } else if (sort == Type.METHOD) { // ... } else { // throw an exception } }
         * else if (cst instanceof Handle) { // ... } else { // throw an exception }
         */
    }
